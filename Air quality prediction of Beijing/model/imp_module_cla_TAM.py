...
class Imp_Single(Module):
    ...
    def forward(self, x, stage, feature_idx=0):
        ...
        x = self.output_linear(x_time=x_time, x_feature=x_feature)
...
class Encoder(Module):
    def __init__(self, in_channel, hidden_len, q, k, v, h, mask, temp_len=512):
        ...
        self.layernorm = torch.nn.LayerNorm(hidden_len)
...
class Output_Layer(Module):
    def __init__(self,
        ...
        self.out_linear = torch.nn.Linear(in_features=(time_len + num_feature) * hidden_len, out_features=num_classes)
    ...
    def forward(self, x_time, x_feature):
        ...
        # 双塔
        x = self.out_linear(torch.cat([x_feature.reshape(x_feature.shape[0], -1), x_time.reshape(x_time.shape[0], -1)], dim=-1))
